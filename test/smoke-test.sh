...
run_expect_failure_message "copy stream requires clip boundary" "--copy-stream requires --start or --end" "$target_script" --copy-stream "$tmp_dir/missing.mp4"
run_expect_failure_message "copy stream and reencode should conflict" "--copy-stream cannot be combined with another operation" "$target_script" --copy-stream --reencode --start 1 "$tmp_dir/missing.mp4" "$tmp_dir/clip.mp4"
...
if "$target_script" --force --start 1 --end 3 "$sample_video" "$default_clip" >/dev/null 2>&1 && [ -f "$default_clip" ]; then
    pass "clip should re-encode by default"
else
    fail "clip should re-encode by default"
fi
if [ -f "$default_clip" ] && [ "$(ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 "$default_clip" | tr -d '\r')" = "h264" ]; then
    pass "default clip should use H.264 codec"
else
    fail "default clip should use H.264 codec"
fi
...
if "$target_script" --force --add-audio "$sample_image" "$sample_audio" "$created_video" >/dev/null 2>&1 && [ -f "$created_video" ] && [ "$(basename "$created_video")" = "$(basename "${sample_audio%.*}").mp4" ]; then
    ...
    fail "image add-audio output should use audio filename"
fi
...
if output=$("$target_script" --force --copy-stream --start 1 --end 3 "$created_video" "$stream_copy_clip" 2>&1) && [ -f "$stream_copy_clip" ]; then
    pass "H.264/AAC clip should be created with --copy-stream"
else
    fail "H.264/AAC clip should be created with --copy-stream"
fi
if case "$output" in *"requested start: 1.000"*"stream-copy keyframe start:"*) true ;; *) false ;; esac; then
    pass "stream-copy clip should report actual keyframe start"
else
    fail "stream-copy clip should report actual keyframe start"
fi
